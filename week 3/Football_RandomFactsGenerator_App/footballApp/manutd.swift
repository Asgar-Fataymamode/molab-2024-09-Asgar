...
struct ManUView: View {
    ...
    @State private var randomImageName: String = "manutd1"  // Default image for Manchester United
    ...
    var body: some View {
        ...
        .navigationTitle("Manchester United")
    }
    ...
    func shuffleFactsAndImages() {
        let club = footballClubs.first(where: { $0.name == "Manchester United" })!  // Ensure this matches exactly
...
struct ManUView_Previews: PreviewProvider {
    static var previews: some View {
        ManUView()  // Correct the preview to show Manchester United's view
    }
}
